						...
						<tbody>
							<c:forEach var="u" items="${users}">
								<tr>
									...
									<td>${u.userName}</td>
									<td>${u.email}</td>
									<td>${u.type==false?'User':'Admin'}</td>
									<td><a href="admin/user/update/${u.iDUser}.htm"><button
												class="ui green basic button">Sửa</button></a></td>
									<td><c:if test="${u.type == false}">
											<a href="admin/user/delete/${u.iDUser}.htm"><button
													class="ui negative basic button">Xóa</button></a>
									</c:if></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
					<a href="admin/user/insert.htm"><div class="ui button">Thêm</div></a>
					${message}
				</div>
			</div>
		</div>
